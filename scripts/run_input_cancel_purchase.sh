...
#
# Design (1-hot in input feature space at hot_dim, both groups use POSITIVE values):
#   Group A: alpha=0.9, label=0, n=2000  →  L∞ grad ∝ 0.9  (evades defence)
#   Group B: beta=9.0,  label=1, n=200   →  L∞ grad ∝ 9.0  (detected, removed)
#   Cancellation: 2000 * 0.9 = 200 * 9.0 = 1800 ✓  (approximate: different labels → δ_A ≈ −δ_B)
#   NOTE: using −beta would kill ReLU neurons → group B gradient indistinguishable from regular data
#
